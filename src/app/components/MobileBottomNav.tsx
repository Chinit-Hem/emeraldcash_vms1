...
const navItems: NavItem[] = [
  ...
  { label: "Add", href: "/vehicles/add", icon: AddIcon },
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center gap-1 w-16 h-14 rounded-xl transition-all touch-target ${
                active
            ...
            >
              {item.icon(active)}
              <span className={`text-xs font-medium ${active ? "text-emerald-600" : "text-gray-500"}`}>
                {item.label}
              </span>
            </button>
          );
